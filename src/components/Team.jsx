  ...
  const teamMembers = [
    ...
    },
    {
      ...
      social: {
        ...
        instagram: "https://instagram.com/alexkumar"
      }
    }
  ]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {teamMembers.map((member) => (
            <div key={member.id} className="group relative">
              <div className="bg-gradient-to-br from-gray-800/40 to-gray-900/60 backdrop-blur-xl border border-cyan-400/20 rounded-2xl p-6 text-center transform group-hover:-translate-y-2 transition-all duration-500 hover:border-cyan-400/40 hover:shadow-lg hover:shadow-cyan-500/20">
                
                {/* Member Image */}
                <div className="relative mb-6">
                  <div className="w-24 h-24 mx-auto rounded-full overflow-hidden border-3 border-cyan-400/30 group-hover:border-cyan-400/60 transition-all duration-300">
                    <div className="w-full h-full bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
                      <span className="text-2xl font-bold text-cyan-400">{member.name.split(' ').map(n => n[0]).join('')}</span>
                    </div>
                  </div>
                  <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/20 via-blue-500/20 to-cyan-600/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl -z-10"></div>
                </div>

                {/* Member Name */}
                <h3 className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 mb-4">
                  {member.name}
                </h3>
                ...
                <div className="flex justify-center space-x-4">
                  {member.social.linkedin && (
                    <a 
                      ...
                      className="w-10 h-10 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-400/30 rounded-lg flex items-center justify-center hover:border-cyan-400/60 hover:bg-gradient-to-r hover:from-cyan-500/40 hover:to-blue-500/40 transition-all duration-300 group/icon"
                    >
                      <svg className="w-5 h-5 text-cyan-400 group-hover/icon:text-white transition-colors duration-300" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {member.social.website && (
                    <a 
                      ...
                      className="w-10 h-10 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-400/30 rounded-lg flex items-center justify-center hover:border-cyan-400/60 hover:bg-gradient-to-r hover:from-cyan-500/40 hover:to-blue-500/40 transition-all duration-300 group/icon"
                    >
                      <svg className="w-5 h-5 text-cyan-400 group-hover/icon:text-white transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
